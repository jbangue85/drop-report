...
class KpiTests(unittest.TestCase):
    ...
    def test_confirmed_profit_uses_only_delivered_orders(self):
        self.conn.executemany(
            """
            INSERT INTO orders
                (id, fecha, producto, estatus, total_orden, ganancia, precio_proveedor_x_cantidad, precio_flete, cantidad)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                (1, "2026-05-02", "Producto A", "ENTREGADO", 100000, None, 45000, 15000, 1),
                (2, "2026-05-02", "Producto A", "ENTREGADO", 100000, 0, 30000, 10000, 1),
                (3, "2026-05-02", "Producto A", "DEVOLUCION", 100000, 999999, 30000, 10000, 1),
                (4, "2026-05-02", "Producto A", "CANCELADO", 100000, 999999, 30000, 10000, 1),
            ],
        )
        self.conn.execute(
            """
        ...
        self.assertEqual(kpis["ganancia_proyectada"], 25000)

    def test_projected_profit_includes_return_shipping_cost(self):
        self.conn.executemany(
            """
            INSERT INTO orders
                (id, fecha, producto, estatus, total_orden, precio_flete, costo_devolucion_flete, cantidad)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                (1, "2026-05-03", "Producto A", "ENTREGADO", 100000, 15000, 0, 1),
                (2, "2026-05-03", "Producto A", "DEVOLUCION", 100000, 10000, 25000, 1),
            ],
        )
        self.conn.commit()
        ...
        self.assertEqual(kpis["ganancia_proyectada"], 50000)
